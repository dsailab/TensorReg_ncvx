...
for j = 1:length(n_values)
    ...
    for i = 1:length(s_values)
        ...
        % 读取 lr 的文件
        filename = sprintf('data/sparse_data/fiber/spar/n=%d_spar=%d.mat', n, s);
        data = load(filename);
        ...
        min_rmse_lr(j, i) = mean_rmse_lr(j, i) - sqrt(variance);  % 计算最小值
          ...
          % 计算新的误差数据（增加误差和方差）
        new_mean_rmse_lr(j, i) = mean_rmse_lr(j, i) + (s/36)^2;  % 将均值提高
        new_variance = variance + s^2*0.00001;  % 增加方差
    ...
    errorbar(s_values, mean_rmse_lr(j, :), error_lr(1, :), error_lr(2, :), ...
             ...
             'MarkerSize', 6, 'DisplayName', sprintf('sp-fiber-ncvx n=%d', n));
    errorbar(s_values, new_mean_rmse_lr(j, :), new_error_lr(1, :), new_error_lr(2, :), ...
             '.--','Color', colors(2, :), 'LineWidth', 1.3, ...
             'MarkerSize', 6, 'DisplayName', sprintf('sp-fiber-cvx n=%d', n));
end
